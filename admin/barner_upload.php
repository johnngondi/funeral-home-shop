...
if(isset($_FILES["barner"]["name"])){
        $name = $_FILES["barner"]["name"];
        $size = $_FILES["barner"]["size"];
    ...
    $allowed_ext = array("png","jpg","gif","jpeg","JPG","PNG","GIF");
     if(in_array($ext, $allowed_ext)){
         if($size < (5120 * 5120)){
            ...
            list($width, $height) = getimagesize($_FILES["barner"]["tmp_name"]);
                   
           if (move_uploaded_file($_FILES["barner"]["tmp_name"], $path)) {

                echo"
                    <script>
						\$('#barner-prog').hide();
						\$('#barner-img').val(\"$new_name\");
                        var \$toastContent = \$('<span> Barner Image Uploaded!</span>').add(\$('<button class=\"btn-flat toast-action\"><i class=\"fa fa-check green-text\"></i></button>'));
