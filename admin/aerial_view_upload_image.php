...
if(isset($_FILES["aerial-view-image-file"]["name"])){
        $name = $_FILES["aerial-view-image-file"]["name"];
        $size = $_FILES["aerial-view-image-file"]["size"];
    ...
    $allowed_ext = array("png","jpg","gif","jpeg","JPG","PNG","GIF");
     if(in_array($ext, $allowed_ext)){
         if($size < (5120 * 5120)){
            ...
            list($width, $height) = getimagesize($_FILES["aerial-view-image-file"]["tmp_name"]);
             
                 
           if (move_uploaded_file($_FILES["aerial-view-image-file"]["tmp_name"], $path)) {

                echo"
                    <script>
                        \$('#aerial-view-image-prog').hide();
                        \$('#aerial-view-img').val(\"$new_name\");
                        var \$toastContent = \$('<span> Aerial View Image Uploaded!</span>').add(\$('<button class=\"btn-flat toast-action\"><i class=\"fa fa-check green-text\"></i></button>'));
